...
class ProcessTimeout(GeminiCliError):
    """进程超时异常"""
...
class CommandNotAllowed(GeminiCliError):
    """不允许的命令异常"""
...
class SessionNotFound(GeminiCliError):
    """会话未找到异常"""
...
class ProcessFailed(GeminiCliError):
    """进程执行失败异常"""
...
class CliNotFoundError(GeminiCliError):
    """gemini-cli 未找到异常"""

    def __init__(self, cli_path: str):
        super().__init__(
            f"gemini-cli executable not found",
            cli_path
        )
        self.cli_path = cli_path


class InvalidResponseFormat(GeminiCliError):
    """无效响应格式异常"""
...
class SessionStorageError(GeminiCliError):
    """会话存储异常"""

...
from .config import get_gemini_cli_config
...
class SessionStore:
    ...
    async def get(self, session_id: str) -> Optional[SessionInfo]:
        ...
        Returns:
            Optional[SessionInfo]: 会话信息，不存在则返回 None
        """
        async with self._lock:
    ...
    async def list(self) -> list[SessionInfo]:
        ...
        Returns:
            list[SessionInfo]: 会话列表，按最后使用时间倒序
        """
        async with self._lock:
            return sorted(

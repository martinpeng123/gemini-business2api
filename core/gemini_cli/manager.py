...
from .config import GeminiCliConfig, get_gemini_cli_config
from .models import (
...
from .process import run_process, stream_process, build_gemini_command, ProcessResult
from .parser import (
    convert_to_openai_response,
    create_openai_stream_chunk,
    parse_stream_line,
    parse_gemini_native_output,
    build_user_prompt
)
...
class GeminiCliManager:
    ...
    def __init__(
        ...
        self.config = config or get_gemini_cli_config()
    ...
    async def execute(self, req: ExecuteRequest) -> ExecuteResponse:
        ...
        if req.response_format == "openai":
            ...
            try:
                parsed = parse_gemini_native_output(result.stdout)
...
# 全局管理器实例
_global_manager: Optional[GeminiCliManager] = None


async def get_gemini_cli_manager() -> GeminiCliManager:
    """获取全局管理器实例"""
    global _global_manager
    if _global_manager is None:
        _global_manager = GeminiCliManager()
        await _global_manager._ensure_initialized()
    return _global_manager


def reset_gemini_cli_manager():
    """重置全局管理器（用于测试）"""
    global _global_manager
    _global_manager = None


# 导出 create_stream_end_chunk 供 manager 使用
from .parser import create_stream_end_chunk
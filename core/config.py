...
class ConfigManager:
    ...
    def load(self):
        ...
        # 优化迁移逻辑：当两个新配置都为空且存在旧 proxy 时，自动迁移旧值
        if not proxy_for_auth and not proxy_for_chat and old_proxy:
            proxy_for_auth = old_proxy
            proxy_for_chat = old_proxy
        else:
            # 保留原有逻辑：仅当新配置为空且存在对应布尔标志时才迁移
        ...
        # 优先使用环境变量，如果环境变量未设置则使用 YAML 值，最后才用默认 False
        browser_headless_env = os.getenv("BROWSER_HEADLESS", "").lower() in ("true", "1", "yes")
        browser_headless_value = browser_headless_env if os.getenv("BROWSER_HEADLESS") else basic_data.get("browser_headless", False)
        ...
        basic_config = BasicConfig(
            ...
            browser_headless=browser_headless_value,
